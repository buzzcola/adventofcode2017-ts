...
namespace Day09 {
    ...
    SAMPLES_2.forEach(stream => {
        let test = solve(stream[0]);
        let pass = test.garbageCount == stream[1];
        console.log(`\t${pass ? 'pass' : 'fail'}: expected:${stream[1]} actual:${test.garbageCount}`);
        if (!pass) {
            pass2 = false;            
        }
    });
    if(pass2){
        console.log(`Part 2 Answer: ${solve(BIG_PROBLEM).garbageCount}`);
    }
}